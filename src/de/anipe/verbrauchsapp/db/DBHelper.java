...
public class DBHelper extends SQLiteOpenHelper {
	...
	private static final String CAR_DATABASE_CREATE = "create table "
			...
			+ CAR_COLUMN_TYPE + " text not null, " 
			+ CAR_COLUMN_BRAND + " text not null, "
			+ CAR_COLUMN_NUMBER + " text not null, "
			+ CAR_COLUMN_STARTKM + " integer not null, "
			+ CAR_COLUMN_FUELTPE + " text not null,"
			+ CAR_COLUMN_IMAGEDATA + " blob);";
	
	private static final String CONSUMPTION_DATABASE_CREATE = "create table "
			+ TABLE_CONSUMPTIONS + "(" 
			+ COLUMN_ID	+ " integer primary key autoincrement, " 
			+ CONSUMPTION_CAR_ID + " integer not null, "
			+ CONSUMPTION_COLUMN_DATE + " text not null, "
			+ CONSUMPTION_COLUMN_REFUELMILEAGE + " integer not null, "
			+ CONSUMPTION_COLUMN_REFUELLITERS + " integer not null, "
			+ CONSUMPTION_COLUMN_REFUELPRICE + " real, "
			+ CONSUMPTION_COLUMN_DRIVENMILEAGE + " integer not null, "
			+ CONSUMPTION_COLUMN_CONSUMPTION + " real not null);";
