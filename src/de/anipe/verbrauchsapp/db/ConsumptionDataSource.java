...
public class ConsumptionDataSource implements Serializable {
	...
	public List<Consumption> getConsumptionCycles(long carId) {
		...
		Cursor cursor = database.rawQuery(queryString, new String[] {String.valueOf(carId)});
		cursor.moveToFirst();
	...
	public long addCar(Car car) {
		...
		values.put(DBHelper.CAR_COLUMN_IMAGEDATA, getByteArrayForBitMap(car.getImage()));
	...
	public long updateCar(Car car) {
		...
		return database.update(DBHelper.TABLE_CARS, values, DBHelper.COLUMN_ID + "=?", new String[] {String.valueOf(car.getCarId())});
	}
	
	public int deleteCar(Car car) {
		try {
			return database.delete(DBHelper.TABLE_CARS, DBHelper.COLUMN_ID + "=?", new String[] {String.valueOf(car.getCarId())});	
		} catch (android.database.SQLException sqe) {
			sqe.printStackTrace();
			return -1;
		}	
	...
	public long storeImageForCar(long carId, Bitmap bitmap) {
		...
		return database.update(DBHelper.TABLE_CARS, values, DBHelper.COLUMN_ID + "=?", new String[] {String.valueOf(carId)});
	}
	...
	public int deleteConsumption(long consumptionId) {
		try {
			return database.delete(DBHelper.TABLE_CONSUMPTIONS, DBHelper.COLUMN_ID + "=" + consumptionId, null);	
		} catch (android.database.SQLException sqe) {
			sqe.printStackTrace();
			return -1;
		}	
	...
	public int deleteConsumptionsForCar(long carId) {
		try {
			return database.delete(DBHelper.TABLE_CONSUMPTIONS, DBHelper.CONSUMPTION_CAR_ID + "=" + carId, null);	
		} catch (android.database.SQLException sqe) {
			sqe.printStackTrace();
			return -1;
		}	
	...
	public long addConsumption(Consumption cycle) { 
		...
		values.put(DBHelper.CONSUMPTION_COLUMN_DATE, getDateTime(cycle.getDate()));
		values.put(DBHelper.CONSUMPTION_COLUMN_REFUELMILEAGE, cycle.getRefuelmileage());
		values.put(DBHelper.CONSUMPTION_COLUMN_REFUELLITERS, cycle.getRefuelliters());
		values.put(DBHelper.CONSUMPTION_COLUMN_REFUELPRICE, cycle.getRefuelprice());
		values.put(DBHelper.CONSUMPTION_COLUMN_DRIVENMILEAGE, cycle.getDrivenmileage());
		values.put(DBHelper.CONSUMPTION_COLUMN_CONSUMPTION, cycle.getConsumption());
		
		try {
			return database.insertOrThrow(DBHelper.TABLE_CONSUMPTIONS, null, values);	
		} catch (android.database.SQLException sqe) {
			return -1;
		}
	}
